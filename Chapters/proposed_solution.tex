...
A custom Python implementation of the Timed Elastic Band (TEB) algorithm was developed, utilizing the \texttt{pyceres} library~\cite{sarlin2025pyceres} for non-linear optimization. \texttt{pyceres} provides minimal Python bindings that expose the core functionality of the highly optimized C++ Ceres Solver~\cite{agarwal2012ceres}, allowing the high-level logic to be orchestrated in Python while maintaining near-native execution speeds.
...
\begin{itemize}
    \item Static Obstacle Course: The UGV navigated a 30-meter path populated with stationary obstacles arranged in offset patterns, requiring the TEB planner to execute smooth kinodynamic avoidance maneuvers.
...
\begin{itemize}
    ...
    \item E-Stop Rate (\%): The percentage of failures that resulted in a proactive emergency stop. This metric provides insight into the reliability of the system's safety nets.
    \item Path Efficiency (Ratio): Measured as the actual distance traveled by the UGV divided by the shortest-path distance generated by the global A* planner. A score of $1.0$ implies the UGV followed the center of the path perfectly.
...
\begin{itemize}
    ...
    \item Control frame frequency (Hz): The number of control commands generated and sent by the algorithm per second.
\end{itemize}
